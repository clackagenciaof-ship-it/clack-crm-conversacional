...
async function findRecipients(context: any, segmentType: string) {
  ...
  if (segmentType === 'propostas_enviadas') {
    const { data: opportunities } = await context.service
      ...
      .eq('stage_name', 'Proposta Enviada')
      .eq('status', 'Aberta')
      .limit(200);
  ...
  let query = context.service.from('contacts').select('id, name, phone, temperature, status').eq('company_id', companyId).limit(200);
...
export async function GET(request: Request) {
  ...
  const { data: campaigns, error: campaignError } = await context.service
...
export async function POST(request: Request) {
  ...
  const cleanRecipients = recipients
    .map((contact: any) => ({ ...contact, phone: normalizePhone(contact.phone) }))
  ...
  if (cleanRecipients.length) {
    const recipientPayload = cleanRecipients.map((contact: any) => ({
      company_id: context.profile.company_id,
  ...
  await context.service.from('atendimento_audit_logs').insert({
    ...
    next_value: { campaign: campaign.name, segmentType, recipients: cleanRecipients.length }
  });

  return Response.json({ ok: true, campaign: { ...campaign, total_recipients: cleanRecipients.length } });
}

import type { Lead, LeadStatus, LeadTemperature, Opportunity, OpportunityStatus, QuickMessage, Task, TaskPriority, TaskStatus } from '@/types/crm';
...
export function mapContactRowToLead(row: ContactRow, index = 0): Lead {
  return {
    id: Number.parseInt(row.id.replace(/\D/g, '').slice(0, 10), 10) || index + 1,
    name: row.name,
    phone: row.phone,
    email: row.email || '',
    city: row.city || '',

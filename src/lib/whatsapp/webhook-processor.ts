...
async function findAccountByPhoneNumberId(supabase: SupabaseServiceClient, phoneNumberId?: string) {
  ...
  if (error) {
    console.error('Falha ao buscar conta WhatsApp.', error);
    return null;
  }
...
async function findContactByPhone(supabase: SupabaseServiceClient, companyId: string, phone: string) {
  ...
  if (error) {
    console.error('Falha ao buscar contatos para vincular WhatsApp.', error);
    return null;
  }
...
async function findOrCreateConversation(params: {
  ...
  if (findError) {
    console.error('Falha ao buscar conversa WhatsApp.', findError);
  }
...
async function processInboundMessage(params: {
  ...
  const conversation = await findOrCreateConversation({
    ...
    customerPhone,
    customerName: contact?.name || contactName || null
  });
  ...
  await supabase
    ...
    .insert({
      ...
      to_phone: displayPhoneNumber || null,
  ...
  await supabase
    .from('whatsapp_conversations')
    .update({
      contact_id: contact?.id || conversation.contact_id || null,
      customer_name: contact?.name || contactName || conversation.customer_name || null,
  ...
  if (contact?.id) {
    await supabase
      ...
      .insert({
        ...
        description: `WhatsApp recebido: ${body}`
      });
  }
}
...
export async function processWhatsAppWebhookPayload(supabase: SupabaseServiceClient, payload: unknown) {
  ...
  for (const entry of entries) {
    for (const change of entry.changes || []) {
      ...
      for (const contact of change.value?.contacts || []) {
        if (contact.wa_id) contactsByPhone.set(normalizePhone(contact.wa_id), contact.profile?.name || null);
      }
      ...
      for (const message of change.value?.messages || []) {
        await processInboundMessage({
          ...
          contactName: contactsByPhone.get(normalizePhone(message.from)) || null
        });
        processedMessages += 1;
      }
    }
  ...
  return {
    companyId: Array.from(processedCompanyIds)[0] || null,
    processedMessages
  };
}

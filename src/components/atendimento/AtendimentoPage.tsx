...
export function AtendimentoPage() {
  ...
  function chooseFlow(flowId: string) { setSelectedFlowId(flowId); const firstStep = flowSteps.filter((step) => step.flow_id === flowId).sort((a, b) => a.position - b.position)[0]; setSelectedStepId(firstStep?.id || ''); if (firstStep?.message) setReplyText(firstStep.message); }
  function chooseStep(stepId: string) { setSelectedStepId(stepId); const step = flowSteps.find((item) => item.id === stepId); if (step?.message) setReplyText(step.message); }
    ...
    <div className="grid two-col">
      ...
      <div className="card pad">
        ...
        <div className="card pad" style={{ marginTop: 16 }}>
          ...
          <p className="notice">Use jornadas salvas em Configurações. A sequência automática executa o próximo passo e registra no histórico.</p>
